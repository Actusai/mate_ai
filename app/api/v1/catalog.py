...
def list_packages_catalog(
    ...
    company_id: int | None = Query(default=None, description="Filtriraj pakete za tvrtku (prema company_type/AR)."),
    company_type: str | None = Query(default=None, description="'authorized_representative' | 'deployer' | 'developer'"),
):
    ...
    if company_id is not None:
        ...
        effective_company_type = "authorized_representative" if (is_ar_flag or is_ar_type) \
    ...
    if effective_company_type:
        if effective_company_type == "authorized_representative":
            packages = [
                p for p in packages
                if bool(getattr(p, "is_ar_only", 0)) and int(getattr(p, "ai_system_limit", 0) or 0) == 0
            ]
        else:
