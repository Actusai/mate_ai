...
class AssessmentApprovalCreate(BaseModel):
    ...
    note: Optional[constr(strip_whitespace=True, max_length=1000)] = Field(
        default=None, description="Optional approval rationale/comment."
    )
...
class AssessmentApprovalOut(BaseModel):
    ...
    note: Optional[str] = Field(
        default=None, description="Optional note provided by the approver."
    )
    approved_at: datetime = Field(
        ..., description="UTC timestamp when the approval was issued."
    )
