...
class RegulatoryDeadline(Base):
    ...
    # Metadata
    name      = Column(String(255), nullable=False, index=True)     # e.g., "AI Act conformity deadline"
    kind      = Column(String(50), nullable=True, index=True)       # e.g., "ai_act_general", "registration", ...
    due_date  = Column(DateTime(timezone=True), nullable=False, index=True)

    # Optional attributes
    severity  = Column(String(20), nullable=True, index=True)       # e.g., low|medium|high|critical
    status    = Column(String(20), nullable=False, default="open", index=True)  # open|done|missed|waived|archived
...
Index("ix_regdl_company_due", RegulatoryDeadline.company_id, RegulatoryDeadline.due_date)
Index("ix_regdl_system_due", RegulatoryDeadline.ai_system_id, RegulatoryDeadline.due_date)
