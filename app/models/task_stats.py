...
from sqlalchemy import Column, Integer, ForeignKey, Date, DateTime, UniqueConstraint, text
from sqlalchemy.orm import relationship, backref
from app.db.base import Base

class TaskStatsDaily(Base):
    __tablename__ = "task_stats_daily"
    __table_args__ = (UniqueConstraint("day", "ai_system_id", name="ux_task_stats_daily"),)
    ...
    company = relationship("Company", backref=backref("task_stats_daily", lazy="selectin"))
    ai_system = relationship("AISystem", backref=backref("task_stats_daily", lazy="selectin"))

class OwnerTaskStatsDaily(Base):
    __tablename__ = "owner_task_stats_daily"
    __table_args__ = (UniqueConstraint("day", "company_id", "owner_user_id", name="ux_owner_task_stats_daily"),)
    ...
    company = relationship("Company", backref=backref("owner_task_stats_daily", lazy="selectin"))
    owner   = relationship("User",    backref=backref("owner_task_stats_daily", lazy="selectin"))
