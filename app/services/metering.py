...
def _increment_daily(
    db: Session, *, company_id: int, endpoint: str, package_id: Optional[int] = None
) -> int:
    """
    ...
    try:
        ...
        if found:
            db.execute(
                text("UPDATE api_metrics_daily SET count = count + 1 WHERE id = :id"),
                {"id": found["id"]},
            )
...
async def meter_api(
    ...
    if request.method.upper() == "OPTIONS" or any(
        path.startswith(p) for p in IGNORED_PATH_PREFIXES
    ):
        yield
        return
    ...
    if quota and used_before >= quota:
        ...
        raise HTTPException(
            status_code=429,
            detail="Monthly API quota exceeded for this company",
            headers=headers,
        )
    ...
    try:
        ...
        try:
            ...
            if successful:
                new_count = _increment_daily(
                    db, company_id=company_id, endpoint=path, package_id=package_id
                )
                if quota:
